...
int read_frame(camera_v4l2_core *camera)
{
   ...
   // time_t start_clock=clock();
    cv::cvtColor(yuv_8bit,rgb,CV_YUV2BGR_YUYV);//40ms
   ...
   // LOG(INFO)<<"convertfloat cost time:"<<(end_clock-start_clock)/(float)CLOCKS_PER_SEC;
    */
    /*
    float * p_Img;
    cudaMallocManaged((void**)&p_Img, 3*height*width*sizeof(float));
...
//    apt_color=new APT::AptColor(height,width,APT::APT_COLOR_GRBG2RGB_12BIT);
    ...
    while(camera->status)
    {
        ...
        for(;;)
        {
            ...
            frame_count++;
            end_clock = (clock()-start_clock)/(float)(float)CLOCKS_PER_SEC;
            if(end_clock>0){
